...
#include "arraymul_improved_version1.c"
      : [h] "+r"(p_h), [x] "+r"(p_x), [y] "+r"(p_y), [add_cnt] "+r"(add_cnt),
        [sub_cnt] "+r"(sub_cnt), [mul_cnt] "+r"(mul_cnt),
  ...
  macro_improved_version1_cycle_count printf(
      "The total cycle count in this program: %.0f\n",
      improved_version1_cycle_count);

  macro_improved_version1_cpu_time printf("CPU time = %f us\n",
